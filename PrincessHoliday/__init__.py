...
def pcr_constellations(month):
    constellations = ["天秤", "天蝎", "射手", "摩羯", "水瓶", "双鱼", "白羊", "金牛", "双子", "巨蟹", "狮子", "处女"]#台服的会战顺序(按月)
    b_constellations = ["水瓶","双鱼","白羊","金牛","双子","巨蟹","狮子","处女","天秤","天蝎","射手","摩羯"]#国服的（预测） 
    if game_server =="cn":
        for x in range(1,13):
            if x == int(month)-1:
...
async def create_resignation_report(bot, event):
    ...
    #绘制刀数柱状图
    recs = ax.bar(x,y,width=0.4,color=['#fd7fb0','#ffeb6b','#7cc6f9','#9999ff','orange'],alpha=0.5)
    ...
    for i in range(0,5):
        rec = recs[i]
        h = rec.get_height()
        plt.text(rec.get_x(), h, f'{int(times_to_boss[i])}刀',fontdict={"size":10})
    buf = BytesIO()
    ...
    for i in range(0,5):
        rec = recs[i]
        h = rec.get_height()
        plt.text(rec.get_x(), h, f'{int(avg_boss_damage[i]/10000)}万',fontdict={"size":10})
    ...
    img.paste(bar_img1, (600,1450), mask=bar_img1.split()[3])
    img.paste(bar_img2, (240,1450), mask=bar_img2.split()[3])
    ...
    add_text(img, row1, position=(340,1058), textsize=35)
    add_text(img, row2, position=(360,1152), textsize=35)
    add_text(img, row3, position=(280,1242), textsize=35)
    add_text(img, row4, position=(780,1073), textsize=35)
    add_text(img, row5, position=(850,1165), textsize=35)
    add_text(img, row6, position=(780,1260), textsize=35)
    add_text(img, year, position=(433,850), textsize=35)
    add_text(img, month, position=(580,850), textsize=35)
    add_text(img, pcr_constellations(month), position=(680,850), textsize=35)
    if len(clanname) <= 7:
        add_text(img, clanname, position=(315+(7-len(clanname))/2*40, 900), textsize=32)
    else:
        add_text(img, clanname, position=(315+(10-len(clanname))/2*30, 908), textsize=25)
    add_text1(img, nickname, position=(375,805), textsize=30)
...
async def create_resignation_report(bot, event):
    ...
    add_text(img, row5, position=(850,1165), textsize=35)
    add_text(img, row6, position=(780,1260), textsize=35)
    add_text(img, year, position=(433,865), textsize=35)
    add_text(img, month, position=(580,865), textsize=35)
    add_text(img, pcr_constellations(month), position=(680,865), textsize=35)
    if len(clanname) <= 7:
        add_text(img, clanname, position=(300+(7-len(clanname))/2*40, 920), textsize=32)
    else:
        add_text(img, clanname, position=(300+(10-len(clanname))/2*30, 927), textsize=25)
    add_text1(img, nickname, position=(375,805), textsize=30)
